...
In `pointfoot-mujoco-sim/simulator.py`, `SimulatorMujoco._export_depth_frame()` renders the full `480x848` MuJoCo `d435` frame every `depth_capture_period_steps`, converts meters to `16UC1` millimeters for ROS1, and publishes on `MJLAB_DEPTH_ROS_TOPIC`. In `rl-deploy-with-python/mjlab_repts_lin_depth.py`, `RosDepthFrameSource` subscribes to the same topic, converts `16UC1` back to meters with scale `0.001`, uses nearest-neighbor resize to obtain `30x53`, removes the bottom 10 rows and leftmost 8 columns, and produces float32 ONNX input `[1, 1, 20, 45]`; range clipping happens externally and normalization happens inside ONNX. It stores only the latest frame behind a lock; pre-cropped policy-sized input is rejected to prevent a second crop. `WheelfootController.compute_actions()` reads `depth_source.frame()` in the policy loop and feeds ONNX inputs `proprio_history`, `actor_command`, `depth`, and `hidden_state_in`; stale or missing frames raise `TimeoutError` according to `MJLAB_DEPTH_TIMEOUT` and `MJLAB_DEPTH_MAX_AGE`. The depth ONNX interface requires `[1, 1, 20, 45]`; legacy `[1, 1, 30, 45]` and `[1, 1, 28, 48]` policies are rejected.
