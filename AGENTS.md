...
In `pointfoot-mujoco-sim/simulator.py`, `SimulatorMujoco._export_depth_frame()` renders the MuJoCo `d435` camera every `depth_capture_period_steps`, converts meters to `16UC1` millimeters for ROS1, and publishes on `MJLAB_DEPTH_ROS_TOPIC`. In `rl-deploy-with-python/mjlab_repts_lin_depth.py`, `RosDepthFrameSource` subscribes to the same topic, converts `16UC1` back to meters with scale `0.001`, crops the left D435 region, clips/resizes to `[1, 1, 30, 45]`, and stores only the latest frame behind a lock. `WheelfootController.compute_actions()` reads `depth_source.frame()` in the policy loop and feeds ONNX inputs `proprio_history`, `actor_command`, `depth`, and `hidden_state_in`; stale or missing frames raise `TimeoutError` according to `MJLAB_DEPTH_TIMEOUT` and `MJLAB_DEPTH_MAX_AGE`. With `MJLAB_DEPTH_OVERLAY=policy`, the same final depth tensor is also published as `32FC1` on `/mjlab/policy/depth_input` for an in-viewer diagnostic overlay; default `off` keeps this path disabled.
