...
class PolicyDepthDebugSink:
    """Best-effort diagnostic transport for the exact ONNX depth tensor."""
...
class Ros1PolicyDepthDebugPublisher(PolicyDepthDebugSink):
    def __init__(self, topic: str):
        ...
        if not rospy.core.is_initialized():
            rospy.init_node("mjlab_policy_depth_debug", anonymous=True, disable_signals=True)
        self._rospy = rospy
        self._image_type = Image
        self._publisher = rospy.Publisher(topic, Image, queue_size=1)

    def publish(self, depth: np.ndarray, inference_seq: int) -> None:
        array = np.asarray(depth)
        if array.shape != DEPTH_INPUT_SHAPE:
            raise ValueError(f"policy depth must have shape {DEPTH_INPUT_SHAPE}, got {array.shape}")
        depth_2d = np.ascontiguousarray(
            array.reshape(DEPTH_HEIGHT, DEPTH_WIDTH), dtype="<f4"
        )
        msg = self._image_type()
        msg.header.seq = int(inference_seq)
        msg.header.stamp = self._rospy.Time.now()
        msg.header.frame_id = "policy_depth_input"
        msg.height = DEPTH_HEIGHT
        msg.width = DEPTH_WIDTH
        msg.encoding = "32FC1"
        msg.is_bigendian = 0
        msg.step = DEPTH_WIDTH * depth_2d.dtype.itemsize
        msg.data = depth_2d.tobytes()
        self._publisher.publish(msg)


def create_policy_depth_debug_sink() -> PolicyDepthDebugSink:
    """Create the optional controller-to-viewer policy-depth debug publisher."""
    overlay_mode = os.getenv("MJLAB_DEPTH_OVERLAY", "off").strip().lower()
    if overlay_mode not in ("off", "policy", "raw"):
        raise ValueError("MJLAB_DEPTH_OVERLAY must be one of: off, policy, raw")
    if overlay_mode != "policy":
        return NullPolicyDepthDebugSink()
    topic = os.getenv("MJLAB_POLICY_DEPTH_TOPIC", "/mjlab/policy/depth_input")
    try:
        return Ros1PolicyDepthDebugPublisher(topic)
    except Exception as exc:
        print(f"Warning: policy depth debug publisher disabled: {exc}")
        return NullPolicyDepthDebugSink()
